...
def fetch_benchmark_history(index_code: str = "000300", days: int = 500) -> list:
    """
    获取指数历史收盘价（东方财富），供风险分析基准对比使用。
    ...
    Returns:
        list: [{date, close, nav}] 按日期升序排列
    """
    try:
        # 上交所指数用 1. 前缀，深交所用 0. 前缀
        if index_code.startswith(("0", "3")) and len(index_code) == 6:
            secid = f"0.{index_code}" if index_code.startswith("3") else f"1.{index_code}"
        else:
        ...
        logging.warning("fetch_benchmark_history(%s) failed: %s", index_code, e)
        return []
