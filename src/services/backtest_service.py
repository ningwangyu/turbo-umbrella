...
def _calc_max_drawdown(details: list, nav_map: list) -> float:
    """
    计算定投期间的最大回撤。

    使用完整净值序列逐日计算：在每个交易日根据已累计份额计算当日市值，
    跟踪历史峰值，得出最大回撤百分比。
    """
    if not details or not nav_map:
        return 0.0

    # 构建日期→份额映射（每笔定投日买入的份额）
    # details 中 date 是字符串，nav_map 中是整数时间戳，统一转 int
    purchase_shares = {}
    for d in details:
        date_int = int(d["date"])
        purchase_shares[date_int] = purchase_shares.get(date_int, 0) + d.get("shares", 0)

    # 从定投第一天开始，在每个交易日计算市值
    first_date = int(details[0]["date"])
    cum_shares = 0.0
    peak = 0.0
    max_dd = 0.0
    started = False

    for date, nav in nav_map:
        if not started:
            if date < first_date:
                continue
            started = True
        # 累加到该日为止的所有已购份额
        if date in purchase_shares:
            cum_shares += purchase_shares[date]
        if cum_shares <= 0:
            continue
        val = cum_shares * nav
        if val > peak:
...
def backtest_fixed_dca(trend: list, amount: float, frequency: str) -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "max_drawdown": _calc_max_drawdown(details, nav_map),
        },
        "details": details,
    }
...
def run_backtest(code: str, amount: float, frequency: str, strategies: list, time_range: str = "3m") -> dict:
    """
    ...
    Args:
        ...
        time_range: 时间区间 1m/3m/6m/1y/2y/3y/all
    ...
    days = range_days.get(time_range, 90)
    if trend and days < 99999:
        last_date = trend[-1]["date"]
        cutoff = last_date - days * 86400000
        trend = [t for t in trend if t["date"] >= cutoff]
    ...
    return {"code": code, "frequency": frequency, "amount": amount, "time_range": time_range, "results": results}
