...
function calc(h, fd) {
    ...
    return { cost: hv - hp, today, totalProfit: hp + today, totalValue: hv + today, changePct: pct, isUpdated };
}
...
export function renderFundList() {
    ...
    holdings.forEach(h => {
        const fd = fundDataCache[h.code], c = calc(h, fd), dir = colorCls(c.changePct), earnDir = colorCls(c.today), sig = signalCache[h.code], earnLabel = c.isUpdated ? "今日收益" : "今日预估";
            ...
            <div class="card-top">
                <div class="card-info">
                    ...
                    <div class="card-data">
                        ...
                        <div class="card-data-col">${fd ? `<div class="card-data-item"><span class="card-data-label">净值</span><span class="card-data-value">${fd.nav}</span></div><div class="card-data-item"><span class="card-data-label">${c.isUpdated ? "实际" : "估值"}</span><span class="card-data-value ${dir}">${fd.estimated_nav} (${fmtMoney(c.changePct)}%)</span></div>` : '<div class="card-data-item"><span class="card-data-label" style="color:#e74c3c">暂无估值</span></div>'}</div>
                    </div>
                </div>
                <div class="card-earnings ${earnDir}"><div class="earn-label">${earnLabel}</div><div class="earn-amount ${earnDir}">${fmtMoney(c.today)}</div></div>
            </div>
            ${signalHTML}
...
export function renderSummary() {
    if (!holdings.length) { renderEmpty(); return; }
    let tv = 0, tp = 0, te = 0, tc = 0, anyUpdated = false;
    holdings.forEach(h => { const c = calc(h, fundDataCache[h.code]); tv += c.totalValue; tp += c.totalProfit; te += c.today; tc += c.cost; if (c.isUpdated) anyUpdated = true; });
    ...
    if ($todayLabel) $todayLabel.textContent = anyUpdated ? "今日收益" : "今日预估";
...
export async function openFundDetailModal(code) {
    ...
    if (fd) {
        ...
        html += `<div class="detail-change ${dir}">${fd.is_nav_updated ? "实际" : "估值"} ${fd.estimated_nav} (${fmtMoney(parseFloat(fd.estimated_change_pct) || 0)}%)</div>`;
    }
    html += `</div></div>`;
    ...
    if (h && c) {
        ...
        html += `<div class="detail-stat"><span class="detail-stat-label">${c.isUpdated ? "今日收益" : "今日预估"}</span><span class="detail-stat-value ${earnDir}">${fmtMoney(c.today)}</span></div>`;
