...
配置来源优先级：
1. config.json 文件（如存在）
2. 代码中的默认值（config.json不存在或缺少某项时使用）
...
AI_BASE_URL = _AI_CFG.get("base_url", "http://120.224.38.132:7361")  # API地址
AI_API_KEY = os.environ.get("AI_API_KEY") or _AI_CFG.get("api_key", "")  # 优先从环境变量读取
AI_MODEL = _AI_CFG.get("model", "gpt-5.5")                           # 模型名称
